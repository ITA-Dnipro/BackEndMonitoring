...
bool CDateTimePathConstructor::UpdatePathToFile(std::string& path)
{
    ...
    char symbol_for_sect = DetermineSectDividingSymbol(path);

    m_updated_path = path;
    std::size_t posistion_last_founded = m_updated_path.find_last_of(
        '.');
    if (posistion_last_founded != std::string::npos)
    {
...
char CDateTimePathConstructor::DetermineSectDividingSymbol(
